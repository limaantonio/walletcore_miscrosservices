...
func (a *AccountDB) FindById(id string) (*entity.Account, error) {
	...
	stmt, err := a.DB.Prepare("SELECT a.id, a.client.id, a.balance, a.created_at, c.id, c.name, c.email, c.created_at FROM account a INNER JOIN client c ON a.client_id = c.id WHERE a.id = ?")
	if err != nil {
		return nil, err
	}
